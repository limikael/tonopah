...
Hand.prototype.checkStraight = function() {
	...
	for (i = 0; i < c.length - 5 + 1; i++) {
		if (Hand.isStraight(c.slice(i, i + 5))) {
			...
			let cards=c.slice(i, i + 5);
			for (let j=0; j<5; j++)
				this.usedCards.push(cards[j]);
...
Hand.prototype.checkFullHouse = function() {
	...
	if (p >= 0 && q >= 0) {
		...
		this.score.push(p);
		this.score.push(q);
		this.usedCards.push(...this.findAllWithValue(p));
		this.usedCards.push(...this.findAllWithValue(q));
		return true;
	}
...
Hand.prototype.checkHighCard = function() {
	...
	for (var i = 0; i < 5; i++) {
		...
		this.usedCards.push(this.cards[i]);
	}
...
/**
 * Find all cards with specific value.
 */
Hand.prototype.findAllWithValue = function(value) {
	let res=[];

	for (let i = 0; i < this.cards.length; i++) {
		if (this.cards[i].getCardValue()==value)
 ...
 */
Hand.prototype.getUsedCardIndices=function() {
	let res=[];

	for (let i=0; i<5; i++)
		res.push(this.originalCards.indexOf(this.usedCards[i]))
